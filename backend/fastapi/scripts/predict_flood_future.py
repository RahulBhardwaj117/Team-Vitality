"""
Enhanced Flood & Heavy Rain Prediction for ANY Time Period (Past or Future)
Uses improved XGBoost model with weather forecasting for future dates
"""
...
forecaster = HistoricalWeatherForecaster(WEATHER_DB_PATH, MONTHLY_RAIN_PATH)

print(f"✓ Loaded historical data: {len(weather_db)} days ({DATA_START.date()} to {DATA_END.date()})")
print(f"✓ Weather forecasting ready for future dates")

# Constants
GW_EXTRACTION_STAGE = 0.89
GW_DEFICIT = 15000.0 
GW_OVEREXPLOITED = 1
...
def predict_date_range(start_date, end_date, scenario='realistic', is_future=False):
    """
    Predict flood risk for any date range (past or future)
    
    Args:
        start_date: Start date
        end_date: End date
        scenario: For future dates - 'optimistic', 'realistic', 'pessimistic'
        is_future: If True, use weather forecasting
    """
    if is_future or start_date > DATA_END:
        # Use weather forecasting for future dates
        ...
        # Forecast in weekly chunks for better accuracy
...
def predict_week(year, week_number, scenario='realistic'):
    """
    Predict flood risk for a specific week (past or future)
    """
    print(f"\n{'='*80}")
    ...
    if is_future:
        print(f"📅 Future date prediction using {scenario} scenario")
    else:
    ...
    if is_future:
        print(f"\n💡 TIP: This is a forecast based on historical patterns.")
        print(f"   Try different scenarios:")
        print(f"   - 'optimistic' for dry conditions")
        print(f"   - 'realistic' for typical conditions (default)")
        print(f"   - 'pessimistic' for wet conditions")
    
    print("\n" + "="*80)

def predict_month(year, month, scenario='realistic'):
    """
    Predict flood risk for a specific month (past or future)
    """
    month_name = datetime(year, month, 1).strftime('%B')
    
    print(f"\n{'='*80}")
    print(f"   FLOOD PREDICTION FOR {month_name.upper()} {year}")
    print(f"{'='*80}\n")
    ...
    print(f"  - Average Risk: {avg_risk:.1f}%")
    print(f"  - Maximum Risk: {max_risk:.1f}%")
    print(f"  - High Risk Days (>70%): {high_risk_days}")
    print(f"  - Total Rainfall: {results['Rainfall'].sum():.1f} mm")
    
    # Risk level
    if avg_risk > 70:
        risk_level = "EXTREME - Significant flooding expected"
    elif avg_risk > 50:
        risk_level = "HIGH - Flood risk present"
    elif avg_risk > 30:
    ...
    if is_future:
        print(f"💡 Forecast based on historical patterns for {month_name}")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("   ENHANCED FLOOD PREDICTION - PAST & FUTURE")
    print("="*80)
    print("\n🔮 Can predict for ANY date (past or future)!")
    print("\nOptions:")
    print("  1. Predict week")
    print("  2. Predict month")
    print("  3. Exit")
    ...
    while True:
        ...
        if choice == '1':
            try:
                ...
                predict_week(year, week, scenario)
            ...
            try:
                ...
                predict_month(year, month, scenario)
            except Exception as e:
                print(f"Error: {e}")
