...
# ================= CONFIGURATION =================
# Use the new balanced model with absolute paths
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_FILE = os.path.join(SCRIPT_DIR, "flood_xgboost_balanced.pkl")
SCALER_FILE = os.path.join(SCRIPT_DIR, "flood_scaler_balanced.pkl")
FEATURES_FILE = os.path.join(SCRIPT_DIR, "flood_features_balanced.json")
WEATHER_DB_FILE = os.path.join(SCRIPT_DIR, "final_weather.csv")
...
def predict_flood_risk(start_date, end_date):
    ...
    with open("prediction_results.txt", "w", encoding="utf-8") as f:
        f.write("\n" + "="*80 + "\n")
        f.write(f"FLOOD RISK FORECAST ({start_date.date()} to {end_date.date()})\n")
        f.write("="*80 + "\n")
        f.write(f"{'Date':<12} | {'Rain(mm)':<10} | {'Soil(%)':<8} | {'Risk Probability':<20} | {'Status'}\n")
        f.write("-" * 80 + "\n")
        ...
        for idx, row in target_df.iterrows():
            ...
            # Determine status label and color (simulated with text)
            if prob > 0.8:
                status = "EXTREME RISK 🚨"
                high_risk_days += 1
            elif prob > 0.6:
            ...
            # Only print if there's some rain or significant risk, or if it's a short period
            if (end_date - start_date).days < 20 or prob > 0.3 or rain > 10:
                line = f"{date_str:<12} | {rain:<10.1f} | {soil:<8.1f} | {prob*100:<6.1f}%             | {status}\n"
                print(line, end='', flush=True)
                f.write(line)
                
        f.write("-" * 80 + "\n")
        f.write(f"Summary: Found {high_risk_days} days with High/Extreme flood risk.\n")
        f.write("="*80 + "\n")
    
    print(f"\nResults saved to prediction_results.txt", flush=True)
