...
DATA_FILE = os.path.join(BASE_DIR, "delhi_weather.csv")
MODEL_DIR = os.path.join(BASE_DIR, "weather_models_xgb")
...
def get_weather_forecast(date_str):
    ...
    for name, filename in MODELS.items():
        ...
        if name == 'RainProb':
            ...
            result['Rain Probability (%)'] = round(float(pred * 100), 1)
        else:
            pred = model.predict(features_scaled)[0]
            if name == 'MaxTemp':
                result['Max Temp (C)'] = round(float(pred), 1)
            elif name == 'MinTemp':
                result['Min Temp (C)'] = round(float(pred), 1)
            elif name == 'Humidity':
                result['Humidity (%)'] = round(float(pred), 1)
            elif name == 'RainAmount':
                result['Rainfall (mm)'] = round(float(max(0, pred)), 2)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if not os.path.exists(SCALER_FILE):
            ...
            print("Usage: python weather_forcast.py <YYYY-MM-DD> or --train")
