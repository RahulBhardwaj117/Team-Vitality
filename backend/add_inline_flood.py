...
# Find the closing </script> tag before chatbot.js
target = """  </script>

  <!-- Chatbot Widget -->"""

if target in content:
    inline_script = """  // ===== FLOOD PREDICTION LOGIC (INLINED TO BYPASS CACHE) =====
    async function runFloodPrediction() {
      const FASTAPI_URL = 'http://localhost:8001';
      const riskMeter = document.querySelector('.flood-section .risk-meter');
      const riskValue = document.querySelector('.flood-section .risk-meter .value');
      const floodDesc = document.querySelector('.flood-section p');
      const floodList = document.querySelector('.flood-section .prediction-list');
  ...
  <!-- Chatbot Widget -->"""
    ...
    content = content.replace(target, inline_script)
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("✅ Successfully added inline flood prediction script to index.html")
else:
